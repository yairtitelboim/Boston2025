...
import { CATEGORIES, getIconForCategory } from './utils/categoryUtils';
import { togglePOIMarkerVisibility, highlightAllActivePOIs } from './utils/poiVisibilityUtils';

// Define the getColorForCategory function directly in this component
const getColorForCategory = (category) => {
  const categoryColors = {
    ...
    bars: '#9b59b6',         // Purple
    shops: '#3498db',        // Blue
    cultural: '#1abc9c',     // Teal
    parks: '#2ecc71',        // Green
    education: '#f1c40f',    // Yellow
    healthcare: '#e67e22',   // Dark Orange
    transportation: '#95a5a6' // Gray
  };

  return categoryColors[category?.toLowerCase()] || '#7f8c8d'; // Default gray if category not found
};
...
// Completely independent POIData2 component with React.forwardRef
const POIData2 = React.forwardRef(function POIData2WithRef({ map, showOSMPOIs, setShowOSMPOIs, showPOIMarkers, setShowPOIMarkers, visible }, ref) {
  // State for controlling the collapsed state
  const [isPOIDataBarCollapsed, setIsPOIDataBarCollapsed] = useState(!visible);

  // Update collapsed state when visible prop changes
  useEffect(() => {
    if (visible) {
      setIsPOIDataBarCollapsed(false);
      console.log('POIData2: Expanded due to visible prop change');
    } else {
      setIsPOIDataBarCollapsed(true);
      console.log('POIData2: Collapsed due to visible prop change');
    }
  }, [visible]);

  // Use the ref to expose methods to parent components
  React.useImperativeHandle(ref, () => ({
    // Expose methods that can be called from parent components
    openDataBar: () => {
      setIsPOIDataBarCollapsed(false);
      console.log('POIData2: openDataBar method called');
    },
    closeDataBar: () => {
      setIsPOIDataBarCollapsed(true);
      console.log('POIData2: closeDataBar method called');
    },
    highlightPOI: (poi) => {
      console.log('POIData2: highlightPOI method called with:', poi);

      // First, make sure the data bar is open
      setIsPOIDataBarCollapsed(false);
      ...
      const category = (poi.category || poi.type || '').toLowerCase();
      console.log('POIData2: Looking for POI in category:', category);

      // If we have a valid category, expand it
      if (category && CATEGORIES.includes(category)) {
        ...
        console.log(`POIData2: Expanded category ${category} to find POI`);

        // Find the POI in our visible POIs
        setTimeout(() => {
          try {
            // Try to find the POI by name in the category
            const poiName = poi.name?.toLowerCase();
            if (poiName) {
              console.log(`POIData2: Searching for POI with name "${poiName}" in category ${category}`);

              // Scroll to and highlight the POI in the DOM
              const poiElements = document.querySelectorAll('.poi-data2-container .poi-item');
              ...
              poiElements.forEach(element => {
                ...
                if (text.includes(poiName)) {
                  ...
                  console.log('POIData2: Found matching POI element:', element);
                }
              });
              ...
              if (foundElement) {
                ...
                foundElement.style.backgroundColor = 'rgba(255, 0, 0, 0.2)';
                foundElement.style.boxShadow = '0 0 0 2px rgba(255, 0, 0, 0.5)';
                foundElement.style.transition = 'all 0.3s ease';

                // Pulse animation
                foundElement.animate(
                  [
                    { transform: 'scale(1)', boxShadow: '0 0 0 2px rgba(255, 0, 0, 0.5)' },
                    { transform: 'scale(1.05)', boxShadow: '0 0 0 3px rgba(255, 0, 0, 0.7)' },
                    { transform: 'scale(1)', boxShadow: '0 0 0 2px rgba(255, 0, 0, 0.5)' }
                  ],
                ...
                // Also highlight the building on the map if coordinates are available
                if (poi.coordinates || poi.lngLat) {
                  const coordinates = poi.coordinates || poi.lngLat;
                  highlightBuildingAtLocation(coordinates, getColorForCategory(category), `highlight-poi-${Date.now()}`);
                }

                return true; // Successfully highlighted
              } else {
                console.log(`POIData2: Could not find POI element with name "${poiName}" in the DOM`);
              }
            }
          } catch (err) {
            console.error('POIData2: Error highlighting POI:', err);
          }
        }, 300); // Give time for the category to expand
      } else {
        console.log(`POIData2: Invalid or missing category for POI: ${category}`);
      }
    }
  }));
  ...
  // Add state to track the selected POI
  ...
  const renderPOIItems = (category) => {
    ...
    // If we have a selected POI and it belongs to this category, show only that POI
    let displayItems = [];
    let hasMoreItems = false;

    if (selectedPOI && (selectedPOI.category === category || selectedPOI.type?.toLowerCase() === category)) {
      // Find the selected POI in the items array
      const foundPOI = items.find(item =>
        item.name === selectedPOI.name ||
        (item.coordinates && selectedPOI.coordinates &&
         item.coordinates[0] === selectedPOI.coordinates[0] &&
         item.coordinates[1] === selectedPOI.coordinates[1])
      );

      if (foundPOI) {
        // Show only the selected POI
        displayItems = [foundPOI];
        hasMoreItems = items.length > 1;
      } else {
        // If we can't find the exact POI, show all items when expanded
        displayItems = showAllItems[category] ? items : [items[0]];
        hasMoreItems = items.length > 1;
      }
    } else {
      // No selected POI, show only the first item (instead of first 3)
      displayItems = showAllItems[category] ? items : [items[0]];
      hasMoreItems = items.length > 1;
    }
    ...
    return (
      <>
        ...
        {/* Show/Hide more items button removed - functionality moved to category header */}
      </>
    );
  };
  ...
  // Only render if visible is true or undefined (for backward compatibility)
  if (visible === false) {
    console.log('POIData2: Not rendering because visible prop is false');
    return null;
  }
  ...
  return (
    <>
      <POIDataBarContainer
        ...
        id="poi-data2-container"
        className="poi-container poi-data2-container"
        style={{
          backgroundColor: 'rgba(22, 22, 22, 0.9)',  // Black background matching POI Graph
          borderColor: '#b71c1c',
          right: '70px',  // Position it differently from the original
        ...
        <POIDataHeader>
          <POIDataTitle style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
            <div style={{ display: 'flex', alignItems: 'center' }}>
              ...
              {/* Toggle for Show All/Less Categories */}
              <span
            ...
            >
              ...
              {/* X icon to collapse POI Data 2 panel */}
              <div
                onClick={() => setIsPOIDataBarCollapsed(true)}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
          ...
          {/* Original collapse button removed - functionality moved to header */}
        </POIDataHeader>

        <POIDataContent>
          {/* Display all categories or just the first three based on toggle state */}
          {(showAllCategories ? sortedCategories : sortedCategories.slice(0, 3)).map((category, index) => {
            ...
            return (
              <POICategory
                ...
                style={{
                  transition: 'transform 0.8s cubic-bezier(0.34, 1.56, 0.64, 1), opacity 0.8s ease, margin-bottom 0.3s ease',
                  ...
                  marginBottom: !showAllCategories ? '12px' : '2px', // More spacing in Less mode, compact in More mode
              ...
              >
                <POICategoryHeader
                  ...
                  style={{
                    ...
                    // Add more padding in Less mode
                    padding: !showAllCategories ? '8px 12px' : '3px 8px',
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', flexGrow: 1 }}>
                    <POICategoryIcon style={{
                      width: !showAllCategories ? '24px' : '16px',
                      height: !showAllCategories ? '24px' : '16px',
                      marginRight: !showAllCategories ? '8px' : '4px',
                      transition: 'width 0.3s ease, height 0.3s ease, margin-right 0.3s ease'
                    }}>
                      {getIconForCategory(category)}
                    </POICategoryIcon>
                    <span style={{
                      fontSize: !showAllCategories ? '14px' : '11px',
                      ...
                      {visiblePOIs[category]?.length > 1 && (
                        <span
                          onClick={(e) => {
                            e.stopPropagation(); // Prevent category toggle
                            toggleShowAllItems(category, e);
                          }}
                          style={{
                            fontSize: '9px',
                            padding: '1px 3px',
                          ...
                          title={showAllItems[category] ? "Show fewer items" : `Show all ${visiblePOIs[category]?.length} items`}
                        >
                          {showAllItems[category] ? "Less" : `All ${visiblePOIs[category]?.length}`}
                        </span>
                      )}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    <POICountBadge
                      ...
                      style={{
                        marginRight: !showAllCategories ? '12px' : '10px',
                        // Calculate width directly based on number of digits
                        width: (() => {
                          const count = poiCounts[category] || 0;
                          const numDigits = count.toString().length;
                          // Each digit gets ~8px + padding, larger in Less mode
                          const digitWidth = !showAllCategories ? 10 : 8;
                          const padding = !showAllCategories ? 20 : 16;
                          return `${Math.max(!showAllCategories ? 40 : 32, (numDigits * digitWidth) + padding)}px`;
                        })(),
                        // Larger font in Less mode
                        fontSize: !showAllCategories ? '12px' : '10px',
                        padding: !showAllCategories ? '2px 8px' : '1px 6px',
                        // Add transition for smooth width changes
                        transition: 'width 0.5s ease-in-out, background-color 0.3s ease, font-size 0.3s ease, padding 0.3s ease',
                ...
                {expandedCategories[category] && (
                  <div style={{
                    marginTop: '8px', /* Further increased from 4px for more space between header and cards */
                    paddingTop: '8px', /* Added top padding */
                    paddingLeft: '10px', /* Kept the same */
                    paddingRight: '4px', /* Kept the same */
                    paddingBottom: '8px', /* Kept the same */
                    animation: 'slideDown 0.3s ease-out forwards',
                    overflow: 'hidden',
                    backgroundColor: 'rgba(0, 0, 0, 0.15)', /* Kept the same */
                    borderRadius: '4px', /* Kept the same */
                  }}>
          ...
          {/* Show More/Less button removed - functionality moved to header */}
        </POIDataContent>
      </POIDataBarContainer>
